...
{
    slo_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->Y));
}
...
{
    uint16_t addr = AbsoluteIndexedAddr(reg, mmap, reg->X);
    uint8_t val = asl_base(reg, CpuMemRead8(mmap, addr));
...
{
    slo_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->X));
}
...
{
    rla_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->Y));
}
...
{
    rla_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->X));
}
...
{
    sre_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->Y));
}
...
{
    sre_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->X));
}
...
{
    rra_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->Y));
}
...
{
    rra_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->X));
}
...
{
    CpuMemWrite8(mmap, AbsoluteIndexedAddr(reg, mmap, reg->Y), reg->A);
}
...
{
    CpuMemWrite8(mmap, AbsoluteIndexedAddr(reg, mmap, reg->X), reg->A);
}
...
{
    dcp_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->Y));
}

static inline void dec_ax(CpuReg* reg, MMap* mmap)
{
    dec_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->X));
}
...
{
    dcp_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->X));
}
...
{
    uint16_t addr = AbsoluteIndexedAddr(reg, mmap, reg->Y);
    uint8_t val = CpuMemRead8(mmap, addr) + 1;
...
{
    inc_base(reg, mmap, AbsoluteIndexedAddr(reg, mmap, reg->X));
}
...
#define CPU_OPCODE_LIST(OPOCDE_X) \
    OPOCDE_X(0x01, ora_ix, 2, 6, IORW) \
    OPOCDE_X(0x03, slo_ix, 2, 8, IORW) \
    OPOCDE_X(0x04, nop2,   2, 3) \
    OPOCDE_X(0x05, ora_z,  2, 3) \
    OPOCDE_X(0x07, slo_z,  2, 5) \
    OPOCDE_X(0x08, php,    1, 3) \
    OPOCDE_X(0x09, ora_c,  2, 2) \
    OPOCDE_X(0x06, asl_z,  2, 5) \
    OPOCDE_X(0x0A, asl,    1, 2) \
    OPOCDE_X(0x0C, nop3,   3, 4) \
    OPOCDE_X(0x0D, ora_a,  3, 4, IORW) \
    OPOCDE_X(0x0E, asl_a,  3, 6, IORW) \
    OPOCDE_X(0x0F, slo_a,  3, 6, IORW) \
    OPOCDE_X(0x10, bpl,    2, 2, PGC)  \
    OPOCDE_X(0x11, ora_iy, 2, 6, IORW) \
    OPOCDE_X(0x13, slo_iy, 2, 8, IORW) \
    OPOCDE_X(0x14, nop4,   2, 4) \
    OPOCDE_X(0x15, ora_zx, 2, 4) \
    OPOCDE_X(0x16, asl_zx, 2, 6) \
    OPOCDE_X(0x17, slo_zx, 2, 6) \
    OPOCDE_X(0x18, clc,    1, 2) \
    OPOCDE_X(0x19, ora_ay, 3, 5, IORW) \
    OPOCDE_X(0x1A, nop0,   1, 2) \
    OPOCDE_X(0x1B, slo_ay, 3, 7, IORW) \
    OPOCDE_X(0x1C, nop6,   3, 4, PGC) \
    OPOCDE_X(0x1D, ora_ax, 3, 5, IORW) \
    OPOCDE_X(0x1E, asl_ax, 3, 7, IORW) \
    OPOCDE_X(0x1F, slo_ax, 3, 7, IORW) \
    OPOCDE_X(0x20, jsr,    3, 6) \
    OPOCDE_X(0x21, and_ix, 2, 6, IORW) \
    OPOCDE_X(0x23, rla_ix, 2, 8, IORW) \
    OPOCDE_X(0x24, bit_z,  2, 3) \
    OPOCDE_X(0x26, rol_z,  2, 5) \
    OPOCDE_X(0x25, and_z,  2, 3) \
    OPOCDE_X(0x27, rla_z,  2, 5) \
    OPOCDE_X(0x28, plp,    1, 4) \
    OPOCDE_X(0x29, and_c,  2, 2) \
    OPOCDE_X(0x2A, rol,    1, 2) \
    OPOCDE_X(0x2C, bit_a,  3, 4, IORW) \
    OPOCDE_X(0x2D, and_a,  3, 4, IORW) \
    OPOCDE_X(0x2E, rol_a,  3, 6, IORW) \
    OPOCDE_X(0x2F, rla_a,  3, 6, IORW) \
    OPOCDE_X(0x30, bmi,    2, 2, PGC) \
    OPOCDE_X(0x31, and_iy, 2, 6, IORW) \
    OPOCDE_X(0x33, rla_iy, 2, 8, IORW) \
    OPOCDE_X(0x34, nop4,   2, 4) \
    OPOCDE_X(0x35, and_zx, 2, 4) \
    OPOCDE_X(0x36, rol_zx, 2, 6) \
    OPOCDE_X(0x37, rla_zx, 2, 6) \
    OPOCDE_X(0x38, sec,    1, 2) \
    OPOCDE_X(0x39, and_ay, 3, 5, IORW) \
    OPOCDE_X(0x3A, nop0,   1, 2) \
    OPOCDE_X(0x3B, rla_ay, 3, 7, IORW) \
    OPOCDE_X(0x3C, nop6,   3, 4, PGC) \
    OPOCDE_X(0x3D, and_ax, 3, 5, IORW) \
    OPOCDE_X(0x3E, rol_ax, 3, 7, IORW) \
    OPOCDE_X(0x3F, rla_ax, 3, 7, IORW) \
    OPOCDE_X(0x40, rti, 1, 6) \
    OPOCDE_X(0x41, eor_ix, 2, 6, IORW) \
    OPOCDE_X(0x43, sre_ix, 2, 8, IORW) \
    OPOCDE_X(0x44, nop2,   2, 3) \
    OPOCDE_X(0x45, eor_z,  2, 3) \
    OPOCDE_X(0x46, lsr_z,  2, 5) \
    OPOCDE_X(0x47, sre_z,  2, 5) \
    OPOCDE_X(0x48, pha,    1, 3) \
    OPOCDE_X(0x49, eor_c,  2, 2) \
    OPOCDE_X(0x4A, lsr,    1, 2) \
    OPOCDE_X(0x4C, jmp,    3, 3) \
    OPOCDE_X(0x4D, eor_a,  3, 4, IORW) \
    OPOCDE_X(0x4E, lsr_a,  3, 6, IORW) \
    OPOCDE_X(0x4F, sre_a,  3, 6, IORW) \
    OPOCDE_X(0x50, bvc,    2, 2, PGC) \
    OPOCDE_X(0x54, nop4,   2, 4) \
    OPOCDE_X(0x51, eor_iy, 2, 6, IORW) \
    OPOCDE_X(0x53, sre_iy, 2, 8, IORW) \
    OPOCDE_X(0x55, eor_zx, 2, 4) \
    OPOCDE_X(0x56, lsr_zx, 2, 6) \
    OPOCDE_X(0x57, sre_zx, 2, 6) \
    OPOCDE_X(0x58, cli,    1, 2) \
    OPOCDE_X(0x59, eor_ay, 3, 5, IORW) \
    OPOCDE_X(0x5A, nop0,   1, 2) \
    OPOCDE_X(0x5B, sre_ay, 3, 7, IORW) \
    OPOCDE_X(0x5C, nop6,   3, 4, PGC) \
    OPOCDE_X(0x5D, eor_ax, 3, 5, IORW) \
    OPOCDE_X(0x5E, lsr_ax, 3, 7, IORW) \
    OPOCDE_X(0x5F, sre_ax, 3, 7, IORW) \
    OPOCDE_X(0x60, rts,    1, 6) \
    OPOCDE_X(0x61, adc_ix, 2, 6, IORW) \
    OPOCDE_X(0x63, rra_ix, 2, 8, IORW) \
    OPOCDE_X(0x64, nop2,   2, 3) \
    OPOCDE_X(0x65, adc_z,  2, 3) \
    OPOCDE_X(0x66, ror_z,  2, 5) \
    OPOCDE_X(0x67, rra_z,  2, 5) \
    OPOCDE_X(0x68, pla,    1, 4) \
    OPOCDE_X(0x69, adc_c,  2, 2) \
    OPOCDE_X(0x6A, ror,    1, 2) \
    OPOCDE_X(0x6C, jmp_i,  3, 5) \
    OPOCDE_X(0x6D, adc_a,  3, 4, IORW) \
    OPOCDE_X(0x6E, ror_a,  3, 6, IORW) \
    OPOCDE_X(0x6F, rra_a,  3, 6, IORW) \
    OPOCDE_X(0x70, bvs,    2, 2, PGC) \
    OPOCDE_X(0x71, adc_iy, 2, 6, IORW) \
    OPOCDE_X(0x73, rra_iy, 2, 8, IORW) \
    OPOCDE_X(0x74, nop4,   2, 4) \
    OPOCDE_X(0x75, adc_zx, 2, 4) \
    OPOCDE_X(0x76, ror_zx, 2, 6) \
    OPOCDE_X(0x77, rra_zx, 2, 6) \
    OPOCDE_X(0x78, sei,    1, 2) \
    OPOCDE_X(0x79, adc_ay, 3, 5, IORW) \
    OPOCDE_X(0x7A, nop0,   1, 2) \
    OPOCDE_X(0x7B, rra_ay, 3, 7, IORW) \
    OPOCDE_X(0x7C, nop6,   3, 4, PGC) \
    OPOCDE_X(0x7D, adc_ax, 3, 5, IORW) \
    OPOCDE_X(0x7E, ror_ax, 3, 7, IORW) \
    OPOCDE_X(0x7F, rra_ax, 3, 7, IORW) \
    OPOCDE_X(0x80, nop5,   2, 2) \
    OPOCDE_X(0x81, sta_ix, 2, 6, IORW) \
    OPOCDE_X(0x83, sax_ix, 2, 6, IORW) \
    OPOCDE_X(0x84, sty_z,  2, 3) \
    OPOCDE_X(0x85, sta_z,  2, 3) \
    OPOCDE_X(0x86, stx_z,  2, 3) \
    OPOCDE_X(0x87, sax_z,  2, 3) \
    OPOCDE_X(0x88, dey,    1, 2) \
    OPOCDE_X(0x8A, txa,    1, 2) \
    OPOCDE_X(0x8C, sty_a,  3, 4, IORW) \
    OPOCDE_X(0x8D, sta_a,  3, 4, IORW) \
    OPOCDE_X(0x8E, stx_a,  3, 4, IORW) \
    OPOCDE_X(0x8F, sax_a,  3, 4, IORW) \
    OPOCDE_X(0x90, bcc,    2, 2, PGC) \
    OPOCDE_X(0x94, sty_zx, 2, 4) \
    OPOCDE_X(0x91, sta_iy, 2, 6, IORW) \
    OPOCDE_X(0x95, sta_zx, 2, 4) \
    OPOCDE_X(0x96, stx_zy, 2, 4) \
    OPOCDE_X(0x97, sax_zy, 2, 4) \
    OPOCDE_X(0x98, tya,    1, 2) \
    OPOCDE_X(0x99, sta_ay, 3, 5, IORW) \
    OPOCDE_X(0x9A, txs,    1, 2) \
    OPOCDE_X(0x9D, sta_ax, 3, 5, IORW) \
    OPOCDE_X(0xA0, ldy_c,  2, 2) \
    OPOCDE_X(0xA1, lda_ix, 2, 6, IORW) \
    OPOCDE_X(0xA2, ldx_c,  2, 2) \
    OPOCDE_X(0xA3, lax_ix, 2, 6, IORW) \
    OPOCDE_X(0xA4, ldy_z,  2, 3) \
    OPOCDE_X(0xA5, lda_z,  2, 3) \
    OPOCDE_X(0xA6, ldx_z,  2, 3) \
    OPOCDE_X(0xA7, lax_z,  2, 3) \
    OPOCDE_X(0xA8, tay,    1, 2) \
    OPOCDE_X(0xA9, lda_c,  2, 2) \
    OPOCDE_X(0xAA, tax,    1, 2) \
    OPOCDE_X(0xAC, ldy_a,  3, 4, IORW) \
    OPOCDE_X(0xAD, lda_a,  3, 4, IORW) \
    OPOCDE_X(0xAE, ldx_a,  3, 4, IORW) \
    OPOCDE_X(0xAF, lax_a,  3, 4, IORW) \
    OPOCDE_X(0xC0, cpy_c,  2, 2) \
    OPOCDE_X(0xC1, cmp_ix, 2, 6, IORW) \
    OPOCDE_X(0xC3, dcp_ix, 2, 8, IORW) \
    OPOCDE_X(0xC4, cpy_z,  2, 3) \
    OPOCDE_X(0xC5, cmp_z,  2, 3) \
    OPOCDE_X(0xC6, dec_z,  2, 5) \
    OPOCDE_X(0xC7, dcp_z,  2, 5) \
    OPOCDE_X(0xC8, iny,    1, 2) \
    OPOCDE_X(0xC9, cmp_c,  2, 2) \
    OPOCDE_X(0xCD, cmp_a,  3, 4, IORW) \
    OPOCDE_X(0xCA, dex,    1, 2) \
    OPOCDE_X(0xCC, cpy_a,  3, 4, IORW) \
    OPOCDE_X(0xCE, dec_a,  3, 6, IORW) \
    OPOCDE_X(0xCF, dcp_a,  3, 6, IORW) \
    OPOCDE_X(0xB0, bcs,    2, 2, PGC) \
    OPOCDE_X(0xB1, lda_iy, 2, 6, IORW) \
    OPOCDE_X(0xB3, lax_iy, 2, 6, IORW) \
    OPOCDE_X(0xB4, ldy_zx, 2, 4) \
    OPOCDE_X(0xB5, lda_zx, 2, 4) \
    OPOCDE_X(0xB6, ldx_zy, 2, 4) \
    OPOCDE_X(0xB7, lax_zy, 2, 4) \
    OPOCDE_X(0xB8, clv,    1, 2) \
    OPOCDE_X(0xB9, lda_ay, 3, 5, IORW) \
    OPOCDE_X(0xBA, tsx,    1, 2) \
    OPOCDE_X(0xBC, ldy_ax, 3, 5, IORW) \
    OPOCDE_X(0xBD, lda_ax, 3, 5, IORW) \
    OPOCDE_X(0xBE, ldx_ay, 3, 5, IORW) \
    OPOCDE_X(0xBF, lax_ay, 3, 5, IORW) \
    OPOCDE_X(0xD0, bne,    2, 2, PGC) \
    OPOCDE_X(0xD1, cmp_iy, 2, 6, IORW) \
    OPOCDE_X(0xD3, dcp_iy, 2, 8, IORW) \
    OPOCDE_X(0xD4, nop4,   2, 4) \
    OPOCDE_X(0xD5, cmp_zx, 2, 4) \
    OPOCDE_X(0xD6, dec_zx, 2, 6) \
    OPOCDE_X(0xD7, dcp_zx, 2, 6) \
    OPOCDE_X(0xD8, cld,    1, 2) \
    OPOCDE_X(0xD9, cmp_ay, 3, 5, IORW) \
    OPOCDE_X(0xDA, nop0,   1, 2) \
    OPOCDE_X(0xDB, dcp_ay, 3, 7, IORW) \
    OPOCDE_X(0xDC, nop6,   3, 4, PGC) \
    OPOCDE_X(0xDD, cmp_ax, 3, 5, IORW) \
    OPOCDE_X(0xDE, dec_ax, 3, 7, IORW) \
    OPOCDE_X(0xDF, dcp_ax, 3, 7, IORW) \
    OPOCDE_X(0xE0, cpx_c,  2, 2) \
    OPOCDE_X(0xE1, sbc_ix, 2, 6, IORW) \
    OPOCDE_X(0xE3, isb_ix, 2, 8, IORW) \
    OPOCDE_X(0xE4, cpx_z,  2, 3) \
    OPOCDE_X(0xE5, sbc_z,  2, 3) \
    OPOCDE_X(0xE6, inc_z,  2, 5) \
    OPOCDE_X(0xE7, isb_z,  2, 5) \
    OPOCDE_X(0xE8, inx,    1, 2) \
    OPOCDE_X(0xE9, sbc_c,  2, 2) \
    OPOCDE_X(0xEA, nop,    1, 2) \
    OPOCDE_X(0xEB, sbc_c,  2, 2) \
    OPOCDE_X(0xEC, cpx_a,  3, 4, IORW) \
    OPOCDE_X(0xEE, inc_a,  3, 6, IORW) \
    OPOCDE_X(0xED, sbc_a,  3, 4, IORW) \
    OPOCDE_X(0xEF, isb_a,  3, 6, IORW) \
    OPOCDE_X(0xF0, beq,    2, 2, PGC) \
    OPOCDE_X(0xF1, sbc_iy, 2, 6, IORW) \
    OPOCDE_X(0xF3, isb_iy, 2, 8, IORW) \
    OPOCDE_X(0xF4, nop4,   2, 4) \
    OPOCDE_X(0xF5, sbc_zx, 2, 4) \
    OPOCDE_X(0xF6, inc_zx, 2, 6) \
    OPOCDE_X(0xF7, isb_zx, 2, 6) \
    OPOCDE_X(0xF8, sed,    1, 2) \
    OPOCDE_X(0xF9, sbc_ay, 3, 5, IORW) \
    OPOCDE_X(0xFA, nop0,   1, 2) \
    OPOCDE_X(0xFB, isb_ay, 3, 7, IORW) \
    OPOCDE_X(0xFC, nop6,   3, 4, PGC) \
    OPOCDE_X(0xFD, sbc_ax, 3, 5, IORW) \
    OPOCDE_X(0xFE, inc_ax, 3, 7, IORW) \
    OPOCDE_X(0xFF, isb_ax, 3, 7, IORW)

#define CPU_UNDEFINED_OPCODE_LIST \
    [0x00] = invalidOpcode, \
    [0x02] = invalidOpcode, \
    [0x0B] = invalidOpcode, \
    [0x12] = invalidOpcode, \
...
#define OPOCDE_RUN_HANDLER(_opcode, _handler, _size, _cycles, ...) \
    static int32_t _op_##_opcode(CpuReg* reg, MMap* mmap) {        \
        int32_t cycles;                                            \
        OpcodeTrace(#_handler, _size, reg, mmap);                  \
        OP_RUN_CHOSE_MACROS(0, ##__VA_ARGS__, OPOCDE_RUN_,         \
                            )(_opcode, _handler, _size, _cycles);  \
        return cycles;                                             \
    }

CPU_OPCODE_LIST(OPOCDE_RUN_HANDLER)

static int32_t invalidOpcode(CpuReg* reg, MMap* mmap)
{
    LuCNesCPU* cpu = CONTAINER_OF(reg, LuCNesCPU, reg);
...
{
    #define OPCODE_ENTRY(_opcode, _handler, _size, _cycles, ...) [_opcode] = _op_##_opcode,

    static int32_t (*opcodeTable[256])(CpuReg*, MMap*) = {
        CPU_UNDEFINED_OPCODE_LIST,
        CPU_OPCODE_LIST(OPCODE_ENTRY)
    };
    return opcodeTable[currOpcode](reg, mmap);
}
